    ...
    T: AsRef<[u8]>,
{
    ...
    let merkle_values = {
        ...
        let (_, decoded_proof) = nom::combinator::all_consuming(nom::combinator::flat_map(
            super::util::nom_scale_compact_usize,
            |num_elems| nom::multi::many_m_n(num_elems, num_elems, super::util::nom_bytes_decode),
        ))(config.proof.as_ref())
        ...
        let merkle_values = decoded_proof
            ...
            .collect::<std::collections::HashMap<_, _, fnv::FnvBuildHasher>>();
    ...
    let mut unvisited_proof_entries =
        (0..merkle_values.len()).collect::<std::collections::HashSet<_, fnv::FnvBuildHasher>>();
