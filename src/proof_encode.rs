...
use std::{borrow::ToOwned as _, vec::Vec};
use std::{array, iter};
...
impl ProofBuilder {
    ...
    pub fn build(mut self) -> impl Iterator<Item = impl AsRef<[u8]> + Clone> {
        ...
        let num_entries_encoded = super::util::encode_scale_compact_usize(entries.len());

        // Add the size of each entry before each entry.
        let entries = entries.into_iter().flat_map(|entry| {
            let len = super::util::encode_scale_compact_usize(entry.len());
