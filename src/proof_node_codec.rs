...
pub fn encode(
    ...
    let storage_value = match decoded.storage_value {
        ...
        StorageValue::Unhashed(storage_value) => {
            before_storage_value.extend_from_slice(
                super::util::encode_scale_compact_usize(storage_value.len()).as_ref(),
            );
            storage_value
        }
    };
    ...
    let children_nodes = decoded
        ...
        .flat_map(|child_value| {
            let size = super::util::encode_scale_compact_usize(child_value.len());
...
pub fn decode(mut node_value: &'_ [u8]) -> Result<Decoded<DecodedPartialKey<'_>>, Error> {
    ...
    let storage_value = match storage_value_hashed {
        Some(false) => {
        let (node_value_update, len) = super::util::nom_scale_compact_usize(node_value)
